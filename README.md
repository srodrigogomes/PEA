...
Vexa (invocação sem destino declarado) → destino assumido: Descarte.
Remover a carta de Nível 0 só é possível quando ela é o topo daquela posição; a Linha de Evolução não desmorona quando isso acontece — o slot fica vazio e pode ser reocupado depois.
1 Invocação por turno é o padrão (confirmado pela existência da carta "Dias de Sol", que dá uma segunda).
O descarte-custo do Fenômeno "Período de Seca" não ativa o Efeito de Descarte — só o descarte obrigatório de fim de turno ativa efeitos, por padrão.
Revelação e Curioso (mesmo efeito, arte diferente) foram tratadas como duas cartas normais e distintas no baralho.
Confirmado: as 12 cartas de Ativação Rápida/Item são exclusivas da Ervilha nesta coleção — arquitetura pronta para receber equivalentes de Caos no futuro sem refatoração.
Kron mantém o autoalvo em seu Efeito de Descarte (atipico, mas assumido intencional/balanceamento).
